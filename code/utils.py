...
from paths import DATA_DIR, DOCUMENT_FPATH, ENV_FPATH


def load_document(document_external_id="yzN0OCQT7hUS"):
    """Loads the document markdown file.
    ...
    """
    document_fpath = Path(os.path.join(DATA_DIR, f"{document_external_id}.md"))

    # Check if file exists
    if not document_fpath.exists():
        raise FileNotFoundError(f"Document file not found: {document_fpath}")

    # Read and return the file content
    try:
        with open(document_fpath, "r", encoding="utf-8") as file:
            return file.read()
    except IOError as e:
        raise IOError(f"Error reading document file: {e}") from e


def load_all_documents(document_dir: str = DATA_DIR) -> list[str]:
    """Loads all the document markdown files in the given directory.

    Returns:
        List of document contents.
    """
    documents = []
    for doc_id in os.listdir(document_dir):
        if doc_id.endswith(".md"):
            documents.append(load_document(doc_id.replace(".md", "")))
    return documents
